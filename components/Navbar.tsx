    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-black/95 backdrop-blur-sm border-b border-gold/20">
      <nav className="max-w-6xl mx-auto px-4 flex items-center justify-between h-16">
        ...
        <ul className="hidden md:flex items-center gap-6">
          {navLinks.map((l) =>
            l.children ? (
              <li key={l.label} className="relative" onMouseEnter={() => setDropdown(l.label)} onMouseLeave={() => setDropdown(null)}>
                <button className="font-display text-sm font-medium tracking-widest text-white/70 uppercase hover:text-gold transition-colors flex items-center gap-1 py-5">
                  {l.label}
                ...
                {dropdown === l.label && (
                  <div className="absolute top-full left-0 bg-black border border-gold/20 min-w-[160px] py-2">
                    {l.children.map((c) => (
                      <Link key={c.href} href={c.href} className="block px-4 py-3 font-display text-xs tracking-widest uppercase text-white/70 hover:text-gold hover:bg-white/5 transition-colors" onClick={() => setDropdown(null)}>
                        {c.label}
                      </Link>
                    ))}
                  </div>
          ...
          <li>
            <Link href="/register" className="font-display text-sm font-bold tracking-widest uppercase bg-gold text-black px-5 py-2 hover:bg-yellow-400 transition-colors">
              Register
            </Link>
          </li>
        </ul>
      ...
      {open && (
        <div className="md:hidden bg-black border-t border-gold/20 px-4 pb-6 pt-4">
          <ul className="flex flex-col gap-2">
            ...
            <li className="pt-3">
              <Link href="/register" className="inline-block font-display text-sm font-bold tracking-widest uppercase bg-gold text-black px-6 py-3">Register Now</Link>
            </li>
          </ul>
        </div>
      )}
